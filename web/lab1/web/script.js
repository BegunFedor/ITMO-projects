...
//    МОДАЛЬНОЕ ОКНО ------------------------------------------------
  ...
  if (canvas) {
    canvas.addEventListener('click', function(event) {
      ...
      if (planeCoords.y > 3 || planeCoords.y < -3){
...
// КЛИК ПО КНОПКЕ -------------------------------------------
  ...
  function validateY() {
    ...
    if (/[a-zA-Zа-яА-Я]/.test(value) || (value.match(/\./g)).length > 1 || (value.match(/-/g)).length > 1 || (value.includes('-') && !value.startsWith('-'))) {
      setError(yInput, 'Неправильно, переписывайте');
      return false;
    }

    if (value.includes(',')) {
      setError(yInput, 'Используйте точку вместо запятой для дробных чисел');
      return false;
    }
  ...
  function drawTargetArea(ctx, width, height, padding, scale, r) {
    ...
    ctx.fillStyle = '#D9B8B8'; // твой мягкий розово-серый
    ctx.strokeStyle = '#BC8F8F';
    ctx.lineWidth = 2;

    // === 1. Прямоугольник (II квадрант) ===
    ctx.beginPath();
    ...
    // === Оси ===
    ctx.strokeStyle = '#9C7373';
    ctx.fillStyle = '#9C7373';
    ctx.font = '12px Arial';
    const axisScale = 30;

    for (let i = -5; i <= 5; i++) {
      if (i === 0) continue;
      const xPos = centerX + i * axisScale;
      const yPos = centerY - i * axisScale;
