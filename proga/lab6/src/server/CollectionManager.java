...
public class CollectionManager {
    ...
    public void save() {
        fileManager.save(studyGroups, fileManager.getFilePath());
    }

    public void save(String filename) {
        fileManager.save(studyGroups, filename); // позволяет задать путь явно
    }
    ...
    public String filterGreaterThanGroupAdmin(Person admin) {
        ...
        return result.length() > 0 ? result.toString() : "Таких элементов не найдено.";
    }
    ...
    public int removeAllByGroupAdmin(Person admin) {
        int sizeBefore = studyGroups.size();
        studyGroups.values().removeIf(group -> group.getGroupAdmin().equals(admin));
        return sizeBefore - studyGroups.size();
    }
}